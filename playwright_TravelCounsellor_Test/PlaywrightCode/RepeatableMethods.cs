...
namespace playwright_TravelCounsellors_Test.PlaywrightCode
{
    public class RepeatableMethods : PageTest
    {
        ...
        public static async Task LaunchLoginUser(IPage Page, string userbrowser, string userdid, string userpassword)
        {
            ...
            await RepeatablePlaywrightMethods.TestTextBox_GetByLabel(Page, pageLogin.Testemail).GetResult().FillAsync(userdid);
            await RepeatablePlaywrightMethods.TestTextBox_GetByLabel(Page, pageLogin.Testpassword).GetResult().FillAsync(userpassword);
